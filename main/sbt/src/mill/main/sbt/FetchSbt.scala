...
// copied from bleep.FetchSbt
@mill.api.internal
class FetchSbt(ec: ExecutionContext) {
  def apply(version: String): Path = {
    ...
    Await.result(cache.get(Artifact(url)).value(ec), Duration.Inf) match {
      case Left(value) => throw new Exception(value, s"couldn't download sbt version $version from url $url")
      case Right(folder) =>
