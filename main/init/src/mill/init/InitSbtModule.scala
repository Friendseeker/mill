...
import coursier.{LocalRepositories, MavenRepository}
...
object InitSbtModule extends ExternalModule with InitSbtModule with TaskModule {

  lazy val millDiscover: Discover = Discover[this.type]
}

/**
 * Defines a [[InitModule.init task]] to convert a sbt build to Mill.
 */
@mill.api.experimental
trait InitSbtModule extends TaskModule {
  ...
  def buildGenClasspath: T[Loose.Agg[PathRef]] = T {
    ...
    Util.millProjectModule("mill-main-sbt", repositories)
  }

  /**
   * Mill build file generator application entrypoint.
   */
  def buildGenMainClass: T[String] = "mill.main.sbt.BuildGen"
  ...
  /**
   * Generates and formats Mill build files for an existing sbt project.
   *
  ...
  def init(args: String*): Command[Unit] = Task.Command {
    ...
    if (exit == 0) {
      ...
      throw InitSbtException(s"$mainClass exit($exit)")
    }
  }
}

@mill.api.experimental
case class InitSbtException(message: String) extends Exception(message) with NoStackTrace
